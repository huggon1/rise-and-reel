...
import {
  advanceCooperativeRound,
  createCooperativeRound,
...
const makeSteadyGame = (): CooperativeState => {
  const game = createCooperativeRound(players, steadyRandom);
  return {
    ...game,
    fish: { ...game.fish, agility: 0, jitter: 0, dartRate: 0 },
    fishX: 0.5,
    fishY: 0.5,
    fishTargetX: 0.5,
    fishTargetY: 0.5,
...
describe("2D cooperative overlap", () => {
...
describe("2D cooperative round lifecycle", () => {
  ...
  it("starts a new shared round while preserving team totals", () => {
    const game = advanceCooperativeRound(
      {
        ...makeSteadyGame(),
        phase: "caught",
        phaseTime: 0.02,
